...
public class Blobs
{
    ...
    public int BlobAnalysis(Mat SrcMat,           // input image
                ...
                int MinArea)                        // minimum region area
    {
        ...
        System.out.println("Looping from " + Row0 + " to " + (Row0 + Rows));
        ...
        for(int irow = Row0; irow < Row0+Rows; irow++)  // index within Src
        {
            ...
            int[] rowData = getRow(SrcMat, jrow);
            int[] prevRowData = null;
            if (jrow > 0)
                prevRowData = getRow(SrcMat, jrow);
            ...
            for(int icol = Col0; icol < Col0+Cols; icol++)  // index within Src
            {
                ...
                LabelA = LabelB = LabelC = LabelD = 0;
                ColorD = rowData[jcol]; // fetch color of cell
            
                if(jrow == 0 || jcol == 0)  // first column or row
                {
                    if(jcol > 0)
                    {
                        ColorC = rowData[jcol - 1];
                        LabelC = LabelMat[jrow][jcol-1];
                    }
                    if(jrow > 0)
                    {
                        ColorB = prevRowData[jcol];
                        LabelB = LabelMat[jrow-1][jcol];
                    }
                }
                else
                {
                    ColorA = prevRowData[jcol - 1]; if(ColorA > 0) ColorA = 1;
                    ColorB = prevRowData[jcol]; if(ColorB > 0) ColorB = 1;
                    ColorC = rowData[jcol - 1]; if(ColorC > 0) ColorC = 1;
    ...
    private static int[] getRow(Mat mat, int y)
    {
        ...
        for (int i = 0; i < elems.length; i++)
            result[0] = Integer.parseInt(elems[i]);
        return result;
    }
}
